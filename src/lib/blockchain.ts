...
export default class Blockchain {
  ...
  public addTransaction(transaction: Transaction): Validation {
    if (transaction.txInputs && transaction.txInputs.length > 0) {
      ...
      if (
        transaction.txInputs.some(
          (txInput) =>
            !utxo.some(
              (txo) =>
                txo.txHash === txInput.previousTxHash &&
                txo.amount >= txInput.amount
            )
        )
      ) {
        return new Validation(
          'The transaction input is not in the UTXO list',
          false
        );
      }
    }
  ...
  public getTxInputs(wallet: string): Array<TransactionInput | undefined> {
    const txs = this.blocks
      ...
      .filter(
        /* c8 ignore next */
        (txInput) => txInput?.fromAddress && txInput.fromAddress === wallet
      );
